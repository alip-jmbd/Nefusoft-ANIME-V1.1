...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const [schRes, ongDataRaw, popDataRaw] = await Promise.all([
        ...
        if (schRes.data && Array.isArray(schRes.data)) {
          schRes.data.forEach(item => {
            ...
            schData[dayKey] = (item.animeList || []).map(a => ({
              ...
              image_cover: a.cover,
              status: "ONGOING" // Assume ongoing if in schedule
            }));
          });
        }

        // Normalize ongoing data
        const ongData = (ongDataRaw || []).map(a => ({
          id: a.id,
          url: a.url,
          title: a.judul,
          image_poster: a.cover,
        ...
        // Normalize popular data
        const popData = (popDataRaw || []).map(a => ({
          id: a.id,
          url: a.url,
          title: a.judul,
          image_poster: a.cover,
    ...
    <div className="min-h-screen bg-[#0a0a0c] font-nunito selection:bg-[#F6CF80] selection:text-black pb-24 text-white relative">
                ...
                <div className="absolute bottom-6 left-6 md:bottom-12 md:left-12 flex items-end gap-4 md:gap-6 z-10 w-[calc(100%-48px)] md:w-[calc(100%-96px)] max-w-7xl mx-auto pr-8 md:pr-0">
                  ...
                  <div className="flex flex-col text-left mb-1 md:mb-2 gap-1 md:gap-1.5 flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 md:mt-2">
                      <button onClick={() => navigate(`/anime/${(a.url || '').replace(/\/$/, '')}`)} className="h-8 md:h-10 px-5 md:px-6 bg-[#F6CF80] hover:bg-[#ebd59b] text-black rounded font-black tracking-wider text-[10px] md:text-xs flex items-center justify-center gap-1.5 shrink-0 transition-colors">
      ...
      <section className="max-w-7xl mx-auto px-6 mt-12">
        ...
        <div ref={ongoingScrollRef} className="flex overflow-x-auto gap-3 pb-4 custom-scrollbar snap-x px-2">
          {isLoading ? [...Array(8)].map((_, i) => <CardSkeleton key={i} />) : 
            ongoing.map((a, i) => (
              <div key={a.id || i} ref={el => ongoingCardRefs.current[i] = el} onClick={() => navigate(`/anime/${(a.url || '').replace(/\/$/, '')}`)} className="min-w-[105px] w-[105px] group cursor-pointer snap-start transition-all duration-700 opacity-0 blur-xl translate-y-4 active:scale-95 flex flex-col gap-2">
      ...
      <section className="max-w-7xl mx-auto px-6 mt-10 relative">
        ...
        <div ref={todayScrollRef} className="flex overflow-x-auto gap-3 pb-4 custom-scrollbar snap-x px-2">
          {isLoading ?[...Array(8)].map((_, i) => <CardSkeleton key={i} />) : 
            todayAnime.map((a, i) => (
              <div key={a.id || i} ref={el => todayCardRefs.current[i] = el} onClick={() => navigate(`/anime/${(a.url || '').replace(/\/$/, '')}`)} className="min-w-[105px] w-[105px] group cursor-pointer snap-start transition-all duration-700 opacity-0 blur-xl translate-y-4 active:scale-95 flex flex-col gap-2">
                <div className="relative aspect-[3/4.5] overflow-hidden bg-[#16161a] rounded-sm shadow-xl">
                  <img src={a.image_poster} referrerPolicy="no-referrer" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                </div>
                <h3 className="text-[9px] font-bold text-white/60 line-clamp-1 capitalize group-hover:text-[#F6CF80] transition-colors">{a.title.toLowerCase()}</h3>
