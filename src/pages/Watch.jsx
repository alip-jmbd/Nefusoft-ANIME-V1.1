...
const Watch = () => {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      ...
      try {
        ...
        if (res.status && res.data) {
          ...
          const shareTitle = `Tonton ${res.data.title} - NimeStream`;
    ...
    return () => {
      updateMetaTags(
        'NimeStream - Streaming Anime Sub Indo Gratis',
        'Nonton ribuan anime subtitle Indonesia secara gratis tanpa gangguan iklan di NimeStream dengan kualitas tinggi.',
  ...
  const handleShare = async (platform) => {
    ...
    const textMsg = `Tonton ${anime?.title || 'Anime'} di NimeStream, Gratis & Tanpa Iklan !!`;
    ...
    if (platform === 'api' && navigator.canShare) {
      try {
        ...
        await navigator.share({ title: 'NimeStream', text: textMsg, url });
      } catch (e) {}
