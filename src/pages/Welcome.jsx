  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (isMounted) setLiveResults(mapped);
      } catch (e) { 
            ...
            <div className="w-full max-w-lg relative">
              ...
              {liveResults.length > 0 && (
                <div className="absolute top-[60px] inset-x-0 bg-white rounded-2xl overflow-hidden z-[100] max-h-64 shadow-2xl overflow-y-auto custom-scrollbar">
                  {liveResults.map(r => (
                    <div key={r.id || r.url} onClick={() => navigate(`/anime/${r.url}`)} className="flex items-center gap-4 p-3 hover:bg-gray-100 border-b border-gray-100 cursor-pointer text-left transition-colors">
