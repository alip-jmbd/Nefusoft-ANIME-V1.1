      ...
      <nav className="fixed top-2 inset-x-4 z-[100] max-w-7xl mx-auto">
        <div className="bg-black/60 h-16 px-6 rounded-2xl flex items-center justify-between border border-white/5 shadow-lg relative overflow-hidden">
          <div className="flex items-center shrink-0 z-10">
            <img src="https://raw.githubusercontent.com/alip-jmbd/alipp/main/icon-rbg.png" className="w-10 md:w-14 aspect-square object-contain cursor-pointer" alt="NimeStream" onClick={() => navigate('/home')} />
          </div>
          ...
          <div className="flex items-center justify-end flex-1 gap-3 z-10">
            ...
            <div onClick={() => user ? handleLogout() : setShowLoginPopup(true)} className="w-9 h-9 bg-white/5 rounded-full flex items-center justify-center text-white cursor-pointer border border-white/10 hover:bg-[#F6CF80] hover:text-black hover:border-[#F6CF80] transition-colors shrink-0 overflow-hidden">
              {user ? (
      ...
      {showLoginPopup && (
        <div className="fixed inset-0 z-[999] bg-[#0a0a0c]/90 flex items-center justify-center p-4 animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-[#16161a] border border-white/10 rounded-3xl p-8 max-w-sm w-full flex flex-col items-center relative shadow-2xl">
            ...
            <img src="https://raw.githubusercontent.com/alip-jmbd/alipp/main/irohaplenger.jpg" alt="Login" className="w-32 h-32 object-cover rounded-full mb-6 shadow-[0_0_30px_rgba(246,207,128,0.15)] border-4 border-[#F6CF80]/20" />
            <h3 className="text-white font-black text-2xl mb-3 text-center tracking-tight">Sudah Punya Akun?</h3>
            <p className="text-white/50 text-sm font-medium text-center leading-relaxed mb-8">Masuk untuk menikmati fitur favorit, riwayat tontonan, dan lainnya!</p>

            <button
              onClick={handleGoogleLogin}
              className="w-full bg-white text-black font-bold py-3 px-6 rounded-xl flex items-center justify-center gap-3 hover:bg-[#F6CF80] transition-all active:scale-95"
            >
